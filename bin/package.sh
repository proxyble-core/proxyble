...
PLATFORM="${PROXYBLE_PACKAGE_PLATFORM:-x86}"
PROXYBLE_SRC_DIR="$PROJECT_DIR/src"
RULE_AGENT_SRC_DIR="$PROJECT_DIR/proxyble-rule-agent"
BUILD_DIR="$(mktemp -d /tmp/proxyble-package.XXXXXX)"
...
[[ "$PLATFORM" =~ ^[A-Za-z0-9._-]+$ ]] || { echo "[ERROR] Platform may only contain letters, numbers, dots, underscores, and hyphens"; exit 1; }
...
echo "[1/4] Building proxyble"
CGO_ENABLED=0 GOCACHE="${GOCACHE:-/tmp/go-build}" go build -C "$PROXYBLE_SRC_DIR" -buildvcs=false -o "$PKG_DIR/proxyble" .

echo "[2/4] Building proxyble-rule-agent"
CGO_ENABLED=0 GOCACHE="${GOCACHE:-/tmp/go-build}" go build -C "$RULE_AGENT_SRC_DIR" -buildvcs=false -o "$PKG_DIR/bin/proxyble-rule-agent" .
...
cp -a "$PROJECT_DIR/bin/riodb-settings.json" "$PKG_DIR/bin/"
